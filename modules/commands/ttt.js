...
module.exports.handleReply = async function({ event, api, handleReply }) {
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    if(checkGameOver(data)) {
      var gayban = ["chicken 😎", "hat 😎", "What age 😎", "a bit immature 😎", "chicken vcl 😎", "easy game 😎"];
...
module.exports.run = async function ({ event, api, args }) {
  ...
  } else return api.sendMessage("This group already exists a chessboard\nUse:\n" + concak + " continue -> continue\n" + concak + " delete -> erase", threadID, messageID);
}