...
module.exports.run = async ({ api, event,args }) => {  {
  ...
  var callback = () => api.sendMessage({body:``,attachment: fs.createReadStream(__dirname + "/cache/biden.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/biden.png"),event.messageID);
	 return request(encodeURI(`https://betabotz-api.herokuapp.com/api/photooxy/smoke?text=${text}&apikey=BetaBotz`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback());     
}}
//https://betabotz-api.herokuapp.com/api/photooxy/smoke?text=${text}&apikey=BetaBotz