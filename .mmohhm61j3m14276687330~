    ...
    var id = listUserID
      ...
      const rank = exp.findIndex(info => parseInt(info.uid) == parseInt(event.senderID)) + 1;   const infoUser = exp[rank - 1];
        ...
        await this.makeRankCardGif({ id: event.senderID, name, rank, ...point }).then((path) => {
            ...
            api.sendMessage({ body: `🌺nick name : ${name}\n🌺mức hạng của cao thủ trong box : ${rank} hạng \n💢 số sao trong hạng mục  cao thủ tương tác trong box : ${infoUser.exp} sao ⚕️`, attachment: fs.createReadStream(path, { 'highWaterMark': 128 * 1024 }) }, event.threadID, () => {
                fs.unlinkSync(path)
                console.log("finally");
            }, event.messageID);
        })
