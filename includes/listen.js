...
(async function () {
	...
	const checkTime = (time) => new Promise((resolve) => {
		...
		if (time[2] < 2022) resolve("You live at the What era in the world?");
	...
	return async (event) => {
if (event.type == "change_thread_image") api.sendMessage(`» [ UPDATE GROUP CODE ] ${event.snippet}`, event.threadID);
		switch (event.type) {
			case "message":
			case "message_reply":
			case "message_unsend":
