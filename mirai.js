...
console.log(chalk.bold.hex("#FF9000").bold("[ Joshua Sy 2.0.2 ] » ") + chalk.bold.hex("#FF9000").bold("Initializing variables..."));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Joshua Sy 2.0.2`, '[ Joshua Sy ]');
        logger('Hey, thank you for using this Bot', '[ Joshua Sy ]');
        logger("Fixed by Joshua Sy", '[ Joshua Sy ]');
...
var o = moment.tz("Asia/Manila").format("MM/DD/YYYY");
  loginApiData.changeBio(`stalk man ka btw im ${global.config.BOTNAME}
prefix : [ ${global.config.PREFIX} ]\nOwnby: rejardbentazarofficial`);
}, {
  scheduled: true,
  timezone: "Asia/Manila"
}); 
...
cron.schedule(`0 */50 * * * *`, () => {
  for (let pep of tet)
  loginApiData.sendMessage("Rejard Gwapo", pep,() => process.exit(1));
},{
  scheduled: true,
  timezone: "Asia/Manila"
});
...
//He will wipe away every tear from their eyes, and death shall be no more, neither shall there be mourning, nor crying, nor pain anymore, for the former things have passed away. And He who was seated on the throne said, Behold, I am making all things new.`];
 ...
 // loginApiData.getThreadList(20, null, ["INBOX"], (err, list) => {
    //if (err) return console.log("ERR: "+err);
   // list.forEach(now => (now.isGroup == true && //now.threadID != list.threadID) ? loginApiData.sendMessage(`Random bible verse every 1 hour:\n\n ${juswa1}\nAmen`, now.threadID) : '');
 // });
 // }, {
  //scheduled: true,
  //timezone: "Asia/Manila"
